...
/* overrride plugin output for the main menu*/
$menu_overrides = array(
	...
	'filters_menu'   => '<a href="?page=filters"><img src="themes/'.$pd->pd['theme'].'/images/filter.png" title="Filters" alt="Filters" /></a>&nbsp;',
	'news_menu'   => '<a href="?page=news"><img src="themes/'.$pd->pd['theme'].'/images/news.png" title="News" alt="News" /></a>&nbsp;',
);
...
echo '
    <head>
        ...
        <link rel="stylesheet" id="css_1" type="text/css" href="themes/'.$pd->pd['theme'].'/css/main.css" />';
        if (in_array($pd->dsp_page, $app_pages)) { echo '<link rel="stylesheet" id="css_2" type="text/css" href="themes/'.$pd->pd['theme'].'/css/'.$pd->dsp_page.'.css" />'; }
        if ($pd->user->ajax_enabled) {
            /* for js development and debugging
...
<!-- Icon, dropdown, and clock -->
        ...
        <div id="icon" title="Hastymail 2">
            ...
            '.$pd->print_icon() ?>
            <div id="dropdown_outer"><div id="dd_inner"><?php echo $pd->print_folder_dropdown($pd->pd['folders']).'</div>
                 <complex-'.$page_id.'><div id="clock_div">'.$pd->print_clock().'</div></complex-'.$page_id.'>
                 <span id="unread_total"><a href="?page=new&amp;mailbox='.urlencode($pd->pd['mailbox']).'">'.$pd->user->str[34].': '.$_SESSION['total_unread'].'</a></span>
               '.do_display_hook('clock') ?>
            </div>
            <?php echo '<complex-'.$page_id.'>'.'<a href="'.$pd->sticky_url.'" id="show_folders" '; if (isset($_SESSION['hide_folder_list']) && $_SESSION['hide_folder_list']) {
...
<!-- main menu --> 
        ...
        <div id="menu"><?php echo '
            <complex-'.$page_id.'>
            <a href="?page=compose&amp;mailbox='.$pd->pd['url_mailbox'].'" onclick="'.$pd->pd['compose_onclick'].'" class="'.$pd->pd['compose_link_class'].'"><img src="themes/'.$pd->pd['theme'].'/images/compose.png" alt="'.$pd->user->str[3].'" title="'.$pd->user->str[3].'" /></a>&#160;
            <a href="?page=search&amp;mailbox='.$pd->pd['url_mailbox'].'" class="'.$pd->pd['search_link_class'].'"><img src="themes/'.$pd->pd['theme'].'/images/search.png" alt="'.$pd->user->str[9].'" title="'.$pd->user->str[9].'" /></a>&#160;
            <a href="?page=contacts&amp;mailbox='.$pd->pd['url_mailbox'].'" class="'.$pd->pd['contacts_link_class'].'"><img src="themes/'.$pd->pd['theme'].'/images/contacts.png" alt="'.$pd->user->str[8].'" title="'.$pd->user->str[8].'" /></a>&#160;
            <a href="?page=options&amp;mailbox='.$pd->pd['url_mailbox'].'" class="'.$pd->pd['options_link_class'].'"><img src="themes/'.$pd->pd['theme'].'/images/options.png" alt="'.$pd->user->str[4].'" title="'.$pd->user->str[4].'" /></a>&#160;
            <a href="?page=profile&amp;mailbox='.$pd->pd['url_mailbox'].'" class="'.$pd->pd['profile_link_class'].'"><img src="themes/'.$pd->pd['theme'].'/images/profile.png" alt="'.$pd->user->str[236].'" title="'.$pd->user->str[236].'" /></a>&#160;
            <a href="?page=folders&amp;mailbox='.$pd->pd['url_mailbox'].'" class="'.$pd->pd['folder_link_class'].'"><img src="themes/'.$pd->pd['theme'].'/images/folders.png" alt="'.$pd->user->str[7].'" title="'.$pd->user->str[7].'" /></a>&#160;
            '.do_display_hook('menu', $menu_overrides).'<a href="?page=logout"><img src="themes/'.$pd->pd['theme'].'/images/quit.png" alt="'.$pd->user->str[5].'" title="'.$pd->user->str[5].'" /></a></complex-'.$page_id.'>
            <simple-'.$page_id.'><br />
            ...
            <a href="?page=compose&amp;mailbox='.$pd->pd['url_mailbox'].'" class="'.$pd->pd['compose_link_class'].'">'.$pd->user->str[3].'</a>&#160;
                ...
                <tr>
                    ...
                    echo 'style="display: none;" '; }?>>
                        ...
                        <?php echo do_display_hook('folder_list_top') ?>
                            <h2>
                                <?php echo $pd->user->str[7] ?>
                            </h2>
                            <div id="folder_outer"><div>
                                <?php echo $pd->print_folder_list($pd->pd['folders']) ?>
                            </div></div>
                            <?php echo do_display_hook('folder_list_bottom') ?>
                            <div id="hide_link"><a href="<?php echo $pd->sticky_url ?>" onclick="hide_folder_list(); return false;"><?php echo $pd->user->str[61] ?></a></div>
                        </div>
...
<!-- user notices/errors  -->
                    ...
                    <div id="content">
                        ...
                        <b class="itop"><b class="i1">&#160;</b><b class="i2">&#160;</b>
                        <b class="i3">&#160;</b><b class="i4">&#160;</b></b>
...
<!-- jump to the page specific template -->
                        ...
                        <b class="ibottom"><b class="i4">&#160;</b><b class="i3">&#160;</b>
                        <b class="i2">&#160;</b><b class="fi">&#160;</b></b>
                        </div>
                        <b class="rbottom"><b class="r4">&#160;</b><b class="r3">&#160;</b>
                        <b class="r2">&#160;</b><b class="r1">&#160;</b></b>
                        </div>
...
<!-- page footer -->
    ...
    <div id="footer">
        <?php if ($pd->user->logged_in) { echo '<div id="top_link">'.$pd->pd['top_link'].'</div>'; } ?>
        <?php echo do_display_hook("footer") ?>
        <complex-<?php echo $page_id ?>>&copy; Hastymail Development Group 2009</complex-<?php echo $page_id ?>>
    </div>
    </body>
</html>
