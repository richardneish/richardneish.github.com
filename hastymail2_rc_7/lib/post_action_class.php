...
function set_post_vars() {
    ...
    'undelete_message' => array(
        'uids'                   => array('array', 1, $str[438]),
        'mailboxes'              => array('array', 0, 'Mailbox List'),
    ),
    'delete_message' => array(
        'uids'                   => array('array', 1, $str[439]),
        'mailboxes'              => array('array', 0, 'Mailbox List'),
    ),
    ...
    'flag_message' => array(
        'uids'                   => array('array', 1, $str[441]),
        'mailboxes'              => array('array', 0, 'Mailbox List'),
    ),
    'unflag_message' => array(
        'uids'                   => array('array', 1, $str[442]),
        'mailboxes'              => array('array', 0, 'Mailbox List'),
    ),
    'read_message' => array(
        'uids'                   => array('array', 1, $str[443]),
        'mailboxes'              => array('array', 0, 'Mailbox List'),
    ),
    'unread_message' => array(
        'uids'                   => array('array', 1, $str[444]),
        'mailboxes'              => array('array', 0, 'Mailbox List'),
    ),
    'move_message' => array(
        'uids'                   => array('array', 1, $str[446]),
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                        if (!empty($box_uids)) {
                            $this->perform_imap_action('DELETE', $box, $box_uids, $trash_folder, false);
                        }
                    }
                }
            }
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                        if (!empty($box_uids)) {
                            $this->perform_imap_action('UNDELETE', $box, $box_uids, false, false);
                        }
                    }
                }
            }
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                            if (!empty($box_uids)) {
                                $this->perform_imap_action('FLAG', $box, $box_uids, false, false);
                            }
                    }
                }
            }
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                        if (!empty($box_uids)) {
                            $this->perform_imap_action('UNFLAG', $box, $box_uids, false, false);
                        }
                    }
                }
            }
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                        if (!empty($box_uids)) {
                            $this->perform_imap_action('READ', $box, $box_uids, false, false);
                        }
                    }
                }
            }
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                        if (!empty($box_uids)) {
                            $this->perform_imap_action('UNREAD', $box, $box_uids, false, false);
                        }
                    }
                }
            }
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                        if (!empty($box_uids)) {
                            $this->perform_imap_action('MOVE', $box, $box_uids, $trash_folder, $post['destination_folder']);
                        }
                    }
                }
            }
            ...
            if (is_array($boxes) && !empty($boxes)) {
                foreach ($boxes as $box) {
                    if (isset($uids[$box])) {
                        $box_uids = $uids[$box];
                        if (!empty($box_uids)) {
                            $this->perform_imap_action('COPY', $box, $box_uids, false, $post['destination_folder']);
                        }
                    }
                }
            }
...
function prev_next_action($action, $form, $post) {
    ...
    if (isset($post['prev_next_action'])) {
        $_SESSION['last_prev_next_folder'] = $post['prev_next_folder'];
        switch ($post['prev_next_action']) {
            case 'copy':
                $this->perform_imap_action('COPY', $post['mailbox'], array($post['uid']), false, $post['prev_next_folder']);
                break;
            case 'move':
                $this->perform_imap_action('MOVE', $post['mailbox'], array($post['uid']), false, $post['prev_next_folder']);
                break;
            case 'expunge':
                $trash_folder = false;
                if (isset($_SESSION['user_settings']['trash_folder']) && $_SESSION['user_settings']['trash_folder']) {
                ...
                $this->perform_imap_action('EXPUNGE', $post['mailbox'], array($post['uid']), $trash_folder, $post['prev_next_folder']);
                break;
            case 'delete':
                $trash_folder = false;
                if (isset($_SESSION['user_settings']['trash_folder']) && $_SESSION['user_settings']['trash_folder']) {
                ...
                $this->perform_imap_action('DELETE', $post['mailbox'], array($post['uid']), $trash_folder, $post['prev_next_folder']);
                break;
            case 'flag':
                $this->perform_imap_action('FLAG', $post['mailbox'], array($post['uid']), false, $post['prev_next_folder']);
                break;
            case 'unread':
                $this->perform_imap_action('UNREAD', $post['mailbox'], array($post['uid']), false, $post['prev_next_folder']);
                break;
        }
    }
...
function form_action_next_action_x($form, $post) {
    global $user;
    if ($user->logged_in) {
        $this->prev_next_action('next', $form, $post);
    }
}
function form_action_prev_action_x($form, $post) {
    global $user;
    if ($user->logged_in) {
        $this->prev_next_action('prev', $form, $post);
    }
}
function form_action_up_action_x($form, $post) {
    global $user;
    if ($user->logged_in) {
