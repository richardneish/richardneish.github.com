...
function print_new_content($cols=array('checkbox_cell', 'indicators_cell', 'subject_cell', 'from_cell', 'date_cell', 'size_cell'), $onclick=false) {
    global $page_id;
    $n = 1;
    $data = '';
    $sid = '';
    ...
    if (!empty($this->pd['new_page_data'])) {
        ...
        $data .= '<table class="unread_mailbox_table" cellpadding="0" cellspacing="0" width="100%">';
        $data .= '<tr><th colspan="2"><complex-'.$page_id.'><input onclick="toggle_all(); return false;" '.
            'id="toggle_all_button" type="submit" name="toggle_all_button" value="X" /></complex-'.$page_id.'></th>';
        $data .= $this->print_mailbox_list_headers($cols);
        $data .= '</tr>';
        foreach ($this->pd['new_page_data'] as $i => $array) {
            ...
            $data .= '<tr><td colspan="6" class="mbx_title_cell">';
            if (isset($array['headers'])) {
                $data .= '<complex-'.$page_id.'><a class="toggle_folder" href="?page=new&amp;mailbox='.
                     urlencode($this->pd['mailbox']).'&amp;toggle_folder='.urlencode($i).'" onclick="toggle_all('.
                     $n.', '.($n + count($array['headers']) - 1).'); return false;">x</a></complex-'.$page_id.'>';
            }
            else {
                $data .= '<a class="toggle_folder" style="visibility: hidden">x</a>';
            }
            $data .= '<h4><a title="'.$this->user->htmlsafe($i, 0, 0, 1).'" href="?page=mailbox'.$sid.'&amp;mailbox='.
                     urlencode($i).'">'.$array['total'].'&#160;&#160;'.$this->user->htmlsafe($this->pd['folders'][$i]['realname'], 0, 0, 1).'</a></h4>';
            ...
            if (!empty($array['headers'])) {
                ...
                $data .= '<tr><td colspan="6" class="mbx_unread_cell"><div class="empty_unread">'.$this->user->str[249].'</div></td></tr>';
            }
        }
        ...
        $data .= '<span id="new_page_meta">Found <b>'.$this->pd['grand_total'].'</b> messages in <b>'.$this->pd['unread_folder_count'].'</b> folders</span>';
        }
    }
    else {
...
function print_edit_new_page_form() {
    ...
    $data = '<complex-'.$page_id.'><form method="post" action="?page=new&amp;mailbox='.urlencode($this->pd['mailbox']).'">'.$this->user->str[246].': <select name="new_page_folder">'.
             ...
             $this->print_folder_option_list($include_list).'</select>&#160;<input type="submit" name="remove_new_page_folder" value="'.$this->user->str[248].'" /></form></complex-'.$page_id.'>'; 
    return $data;
}
}
?>
