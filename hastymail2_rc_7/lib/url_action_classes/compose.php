...
function url_action_compose($get) {
    ...
    if ($user->logged_in) {
        if (isset($get['thumbnail']) && isset($get['attachment_id'])) {      
            ...
            if (isset($_SESSION['attachments'][$get['attachment_id']])) {   
                $vals = $_SESSION['attachments'][$get['attachment_id']];    
                if (substr($path, -1) != $fd) {   
        ...
        if (!$user->redirected && isset($get['uid'])) {
            ...
            if ($uid && $mailbox) {
                ...
                if ($status) {
                    ...
                    $message_data = $this->find_message_part($struct, $part);
                    ...
                    switch ($type) {
                        case 'forward_attach':
                            $parts = $this->find_message_part($struct, 0);
                            $filename = false;
                            foreach ($main_headers as $vals) {
                                if (strtolower($vals[0]) == 'subject') {
                                    $filename = substr(preg_replace("/\s+/", '_', trim(preg_replace("/[^a-zA-Z0-9 ]/", ' ', $vals[1]))), 0, 200).'.eml';
                                    break;
                                }
                            }
                            if (!$filename) {
                            ...
                            $this->add_forwarded_attachments(array(0 => array(
                                'encoding' => 'none', 'type' => 'message', 'subtype' => 'rfc822', 'filename' => $filename)), $uid);
                            break;
                        case 'forward':
                            $fparts = array();
                            ...
                            if (!empty($fparts)) {
                                $this->add_forwarded_attachments($fparts, $uid);
                            }
                            if (isset($all_headers['subject'])) {
                                $subject = 'Fw: '.$all_headers['subject'];
                            }
                            if ($message_data['type'] == 'text') {
                                $data = $imap->get_message_part($uid, $part);
                                if ($message_data['encoding'] == 'base64') {
                                    $data = base64_decode($data);
                                }
                                elseif (strtolower($message_data['encoding']) == 'quoted-printable') {
                        ...
                        case 'resume':
                            if (isset($message_data['type']) && $message_data['type'] == 'text') {
                                ...
                                elseif ($message_data['encoding'] == 'quoted-printable') {
                                    $data = $user->user_action->quoted_decode($data);
                                }
                                if ($message_data['subtype'] == 'html') {
                                    $data = html_2_text($data);
                        ...
                        case 'reply':
                            ...
                            if ($type == 'all') {
                                if (isset($all_headers['to'])) {
                                    if (strstr($all_headers['to'], ' ')) {
                                        $bits = explode(' ', $all_headers['to']);
                                    }
                                    else {
                                        $bits = array($all_headers['to']);
                                    }
                                    foreach ($bits as $v) {
                                        if (strstr($v, '@') && !strstr($to, $v)) {
                                            $to .= ', '.rtrim($v, ',');
                                        }
                                    }
                                }
                                if (isset($all_headers['from'])) {
                                    if (strstr($all_headers['from'], ' ')) {
                                        $bits = explode(' ', $all_headers['from']);
                                    }
                                    else {
                                        $bits = array($all_headers['from']);
                                    }
                                    foreach ($bits as $v) {
                                        if (strstr($v, '@') && !strstr($to, $v)) {
                                            $to .= ', '.$v;
	...
	if (isset($get['url'])) {
	    ...
	    //Parse URL now for "to", "subject", and "body" parts, ignore others
	    $url_a = parse_url($url);
        ...
        if (isset($_SESSION['user_settings']['profiles'][$profile_id]['auto_sig']) &&
            $_SESSION['user_settings']['profiles'][$profile_id]['auto_sig']) {
            $message .= "\r\n".$_SESSION['user_settings']['profiles'][$profile_id]['profile_sig'];
        }
        ...
        if (isset($_SESSION['attachments']) && !empty($_SESSION['attachments'])) {
            $user->page_data['attachments'] = $_SESSION['attachments'];
        }
        $user->page_data['folders'] = $_SESSION['folders'];
    }
}
function add_forwarded_attachments($parts, $uid) {
    global $conf;
    global $imap;
    global $fd;
    $path = $conf['attachments_path'];
    foreach ($parts as $id => $vals) {
        ...
        if (is_resource($output_file)) {
            ...
            $attributes = array('time' => time(), 'realname' => $filename, 'filename' => $output_id, 'size' => $size, 'type' => $vals['type'].'/'.$vals['subtype']);
            $_SESSION['attachments'][$output_id] = $attributes;
        }
    }
}
function add_attachemnt($name, $type=false) {
    global $conf;
    global $fd;
    $path = $conf['attachments_path'];
                    ...
                    if (is_resource($input_file)) {
                        ...
                        if (is_resource($output_file)) {
                            ...
                            $attributes = array('time' => time(), 'realname' => $filename,
                                        'filename' => $output_id, 'size' => $size, 'type' => $type,
                            );
                            $_SESSION['attachments'][$output_id] = $attributes;
                            $error = false;
                        }
...
function prep_reply($string) {
    ...
    foreach ($lines as $line) {
        if (hm_strlen($line) + 2 > $max) {
            ...
            $new_lines[] = '> '.rtrim(wordwrap($line, $max, $split, true));
        }
        else {
            if (preg_match('/^\s\S/', $line)) {
                $line = substr($line, 1);
...
function print_compose_form() {
    ...
    $data = '<div id="compose_form"><input type="hidden" id="message_id" name="message_id" value="'.$this->user->htmlsafe($message_id).'" />';
    ...
    foreach ($this->pd['profiles'] as $i => $vals) {
        ...
        $data .= 'value="'.$i.'">'.$vals['profile_name'].'  &#160;&#160;&lt;'.$vals['profile_address'].'&gt;</option>';
    }
    $data .= '</select></td></tr><tr><td class="aleft">'.$this->user->str[55].':</td><td colspan="2"><input type="text" tabindex="1" name="compose_to" id="compose_to" class="address_fld" value="'.
             $this->user->htmlsafe($compose_to, false, true, false, true).'" /></td></tr><tr><td class="aleft">'.$this->user->str[54].':</td><td colspan="2"><input type="text" tabindex="2" '.
             'name="compose_cc" id="compose_cc" class="address_fld" value="'.$this->user->htmlsafe($compose_cc, false, true, false, true).'" /></td></tr><tr><td class="aleft">'.
             ...
             false, true, false, true).'" /></td></tr><tr><td class="sleft">'.$this->user->str[13].':</td><td class="sright" colspan="2"><input type="text" tabindex="4" name="compose_subject" '.
    ...
    $data .= 'name="compose_skip_sent" /></td><td class="send_row"><input type="submit" name="compose_sign" value="'.$this->user->str[17].'" /> &#160;<input type="submit" name="compose_save" '.
             'value="'.$this->user->str[18].'" /> &#160;<input type="submit" tabindex="6" name="compose_send" value="'.$this->user->str[19].'" class="send_button" /></td></tr>'.
             ...
             '<input class="attach" type="submit" name="compose_attach" value="'.$this->user->str[60].'" /></td></complex-'.$page_id.'><simple-'.$page_id.'><td></td></simple-'.$page_id.'><td class="send_row"><input type="submit" name="compose_sign" value="'.$this->user->str[17].'" /> '.
             '&#160;<input type="submit" name="compose_save" value="'.$this->user->str[18].'" /> &#160;<input type="submit" name="compose_send" '.
             'value="'.$this->user->str[19].'" class="send_button" /></td></tr>';
        ...
        foreach ($this->pd['attachments'] as $i => $vals) {
            ...
            if (isset($message_part_types[strtolower($vals['type'])]) && strtolower(substr($vals['type'], 0, 5)) == 'image') {
                if (isset($this->pd['settings']['image_thumbs']) && $this->pd['settings']['image_thumbs']) {
                    ...
                    $data .= '<td><img src="?page=compose&amp;thumbnail=1&amp;rand='.$page_id.'&amp;attachment_id='.urlencode($i).$sess.'" /></td>';
                }
            }
            $data .= '</tr>';
        }
