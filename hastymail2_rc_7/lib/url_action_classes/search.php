    ...
    if ($user->logged_in) {
        ...
        if (isset($_SESSION['search_total'])) {
            if ($_SESSION['search_total'] > 15) {
                $user->page_data['top_link'] = '<br /><a href="'.$user->sticky_url.'#top">'.$user->str[186].'</a>';
            }
...
function print_search_res($cols=array('checkbox_cell', 'indicators_cell', 'subject_cell', 'from_cell', 'date_cell', 'size_cell'), $onclick=false) {
    $n = 1;
    ...
    if (!$empty) {
        $data .= '<table class="search_res_table" cellpadding="0" cellspacing="0" width="100%"><tr><th colspan="2">
                 <input onclick="toggle_all(); return false;" id="toggle_all_button" type="submit" name="toggle_all_button" value="X" /></th>';
        $data .= $this->print_mailbox_list_headers($cols).'</tr>';
        foreach ($this->pd['search_results'] as $i => $array) {
            if (!isset($array['total'])) {
                $array['total'] = 0;
            }
            $data .= '<tr><td colspan="6"><h4>'.$array['total'].'&#160;&#160;<a title="'.$this->user->htmlsafe($i, 0, 0, 1).'" href="?page=mailbox&amp;mailbox='.urlencode($i).'">'.
                $this->user->htmlsafe($this->pd['folders'][$i]['basename'], 0, 0, 1).'</a></h4></td></tr>';
            if (!empty($array['headers'])) {
                $data .= $this->print_mailbox_list_rows($cols, $array['headers'], $onclick, $i, $n);
                $n += count($array['headers']);
            }
            else {
                $data .= '<tr><td colspan="5"><div class="empty_unread">No matching search results in this folder</div></td></tr>';
            }
        }
...
function print_search_form() {
    ...
    for ($i=0;$i<$max;$i++) {
        if ($i == 0) {
            ...
            $data .= 'type="radio" name="and_or_'.$i.'" value="2" /></th></tr><tr><td></td><td><tr><th class="top_row">'.($i + 1).'. '.$this->user->str[96].'</th><td>';
        }
        ...
        $data .= '<tr><th>'.$this->user->str[102].'</th><td><select name="search_fld_1_'.$i.'"><option value="1">'.$this->user->str[106].'</option>';
        ...
        $data .= '</select></td></tr><tr><th>'.$this->user->str[103].'</th><td><select name="search_flags_1_'.$i.'"><option value="1">'.$this->user->str[113].'</option>';
        ...
        $data .= '<table cellpadding="0" style="width: 300px;" cellspacing="0"><tr><td class="button_row" colspan="2">';
    }
    $data .= '</td></tr><tr><td colspan="2">';
    if ($i < $conf['search_max']) {
        $data .= '<input class="search_button alt_button" type="submit" name="full_search" value="'.$this->user->str[9].'" />'.
