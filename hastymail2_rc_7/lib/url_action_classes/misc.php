...
function url_action_about($get) {
    ...
    if ($user->logged_in) {
        ...
        if ($imap->use_folder_cache) {
            $user->page_data['fcache_flag'] = 'yes';
        }
        else {
            $user->page_data['fcache_flag'] = 'no';
        }
        if ($imap->use_uid_cache) {
            $user->page_data['ucache_flag'] = 'yes';
        }
        else {
            $user->page_data['ucache_flag'] = 'no';
        }
        if ($imap->use_header_cache) {
            $user->page_data['hcache_flag'] = 'yes';
        }
        else {
            $user->page_data['hcache_flag'] = 'no';
        }
        if ($user->ajax_enabled) {
            $user->page_data['ajax_flag'] = 'yes';
        }
        else {
            $user->page_data['ajax_flag'] = 'no';
        }
        if (isset($_SERVER['HTTP_HOST'])) {
            $user->page_data['host'] = $_SERVER['HTTP_HOST'];
        }
        else {
            $user->page_data['host'] = 'unknown';
        }
        if (isset($_SERVER['SERVER_ADMIN'])) {
            $user->page_data['admin'] = $_SERVER['SERVER_ADMIN'];
        }
        else {
            $user->page_data['admin'] = 'unknown';
        }
        if (isset($_SERVER['SERVER_SOFTWARE'])) {
            $user->page_data['server'] = $_SERVER['SERVER_SOFTWARE'];
        }
        else {
            $user->page_data['server'] = 'unknown';
        }
        if (isset($_SERVER['HTTP_USER_AGENT'])) {
            $user->page_data['browser'] = $_SERVER['HTTP_USER_AGENT'];
        }
        else {
            $user->page_data['browser'] = 'unknown';
        }
        if (isset($_SERVER['REMOTE_ADDR'])) {
            $user->page_data['ip'] = $_SERVER['REMOTE_ADDR'];
        }
        else {
            $user->page_data['ip'] = 'unknown';
        }
    }
}
}
...
function print_folder_controls() {
    ...
    $data .= '<tr><td>'.$this->user->str[251].'</td><td><select name="delete_folder_name"><option value="">&#160;</option>'.$this->print_folder_option_list($this->pd['folders'],
            ...
            '<option value="">&#160;</option>'.$this->print_folder_option_list($this->pd['folders'], false, 0, array(), true, false, 'all').
            '</select> '.$this->user->str[55].' <input type="text" name="new_folder_name" value="" /></td><td><input type="submit" name="rename_folder" value="'.$this->user->str[255].'" /></td></tr>';
    return $data;
}
...
function print_profile_form() {
    ...
    $data .= '<form method="post" action="?page=profile"><h4>Identity '.(1 + $i).'</h4><input type="hidden" name="profile_count" value="'.$count.'" />'.
             ...
             'class="profile_text" name="profile_name" value="'.$profile_name.'" /></td></tr><tr><th>'.$this->user->str[12].'</th><td><input class="profile_text" type="text" '.
             'name="profile_address" value="'.$profile_address.'" /></td></tr><tr><th>'.$this->user->str[237].':</th><td><input class="profile_text" type="text" '.
    ...
    $data .= '/></td></tr><tr><th>'.$this->user->str[241].'</th><td><input type="radio" name="profile_default" value="1" ';
    ...
    $data .= '<div class="profile_buttons"><input type="submit" name="add_profile" value="'.$this->user->str[243].'" /> &#160;&#160; <a href="?page=options">'.$this->user->str[244].'</a></div></form></div>';
    return $data;
}
}
?>
