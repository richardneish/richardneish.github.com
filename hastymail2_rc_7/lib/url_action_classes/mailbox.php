...
function url_action_mailbox($get) {
    ...
    if ($user->logged_in) {
        ...
        $user->page_data['sort_filters'] = array( 'ALL' => $user->str[113],
            'UNSEEN' => $user->str[114], 'SEEN' => $user->str[115], 'FLAGGED' => $user->str[116], 'UNFLAGGED' => $user->str[117],
            'ANSWERED' => $user->str[118], 'UNANSWERED' => $user->str[119], 'DELETED' => $user->str[295], 'UNDELETED' => $user->str[296]);
        ...
        if (isset($get['track_mail'])) {
            $user->sticky_url = preg_replace("/\&amp;track_mail=(1|0)/", '', $user->sticky_url);
            if ($get['track_mail']) {
...
function print_track_mailbox_link() {
    if (isset($this->pd['settings']['folder_check']) && is_array($this->pd['settings']['folder_check']) &&
        ...
        $data = '<a href="'.$this->sticky_url.'&amp;track_mail=0">'.$this->user->str[31].'</a>';
    }
    else {
        $data = '<a href="'.$this->sticky_url.'&amp;track_mail=1">'.$this->user->str[32].'</a>';
    }
    return $data;
}
}
