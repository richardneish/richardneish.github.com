...
class vcard {
    ...
    function get_quick_list($sort='sort_name', $page=1, $source='local', $filter=false) {
        ...
        if (!$filter && isset($_SESSION['active_contact_source']) && $_SESSION['active_contact_source'] === $source && 
            ...
            if ($source !== 'local') {
                do_work_hook('compose_contact_list', array($filter));
            }
            ...
            if ($page > 1) {
                $res = array_slice($res, ($contacts_per_page*($page - 1)), $contacts_per_page);
            }
            elseif ($page) {
                $res = array_slice($res, 0, $contacts_per_page);
            }
        }
    ...
    function get_card_list($sort=false, $page=0) {
        ...
        if ($this->sort_fld) {
            usort($this->card_list, array($this, 'full_sort_cards'));
            array_unshift($this->card_list, 'nothing');
            unset($this->card_list[0]);
        }
            ...
            if (isset($parts[0])) {
                switch (strtoupper($parts[0])) {
                    ...
                    case 'TEL':
                        ...
                        if (substr($v, 3, 1) == ';') {
                            if (strstr($parts[0], ',')) {
                                $prop = split(',', str_replace('TYPE=', '', strtoupper($parts[0])));
                            }
                            else {
    ...
    function write_cards_db() {
        ...
        if (is_object($dbase)) {
            ...
            if (isset($exists[0]['id'])) {
                $sql = 'update contacts set contacts='.$dbase->qt(serialize($this->card_list)).' where id='.$exists[0]['id'];
                $dbase->update($sql);
                $res = 1;
            }
            else {
                $sql = 'insert into contacts (username, contacts) values('.$dbase->qt($user->username).', '.$dbase->qt(serialize($this->card_list)).')';
    ...
    function write_cards_file() {
        ...
        if (@fwrite($handle, serialize($this->card_list))) {
            @fclose($handle);
            $return = true;
        }
        else {
