        ...
        if ($total > $max) {
            ...
            if ($approx > 400) {
                ...
                foreach ($data as $name => $val) {
                    if (isset($_SESSION['frozen_folders'][$name])) {
                        continue;
                    }
...
class imap extends imap_bodystruct {
    ...
    function get_folders($force=false) {
        ...
        $excluded = array();
        $parents = array();
        array_unshift($this->folder_namespace, $this->folder_prefix);
        foreach ($this->folder_namespace as $namespace) {
            ...
            foreach ($result as $vals) {
                ...
                if (isset($folder_parts[(count($folder_parts) - 2)]) && strtoupper($folder_parts[(count($folder_parts) - 2)]) != 'INBOX') {
                ...
                if ($folder != $namespace && stristr($flags, 'noselect')) { 
                    $no_select = true;
                }
        ...
        $place_holders = array();
        foreach ($parents as $val => $parent) {
            $found = false;
            foreach ($folders as $i => $vals) {
                if ($vals['name'] == $parent['name'].$val) {
                    ...
                    foreach ($parent['name_parts'] as $i => $v) {
                        ...
                        if (!isset($folders[$fname])) {
                            ...
                            $folders[$fname] = array('parent' => $fparent, 'delim' => $delim, 'name' => $freal,
                                            ...
                                            'check_for_new' => false, 'sort_by' => false);
                        }
                    }
                }
            }
        ...
        if (!isset($folders['INBOX']) && $this->max_read) {
            $folders = array_merge(array('INBOX' => array(
                    'name' => 'INBOX', 'basename' => 'INBOX', 'realname' => 'INBOX',
        ...
        if ($page) {
            if ($this->use_header_cache) {
                if (isset($_SESSION['header_cache'][$mailbox][$page])) {
                    if (!isset($_SESSION['header_cache_refresh'][$mailbox])) {
    ...
    function get_message_headers($uid, $message_part) {
        if ($message_part == 1 || !$message_part) {
            ...
            $command = "UID FETCH $uid BODY[$message_part.HEADER]\r\n";
        }
        $this->send_command($command);
        $result = $this->get_response(false, true);
        $status = $this->check_response($result, true);
    ...
    function message_action($uids, $action, $mailbox=false, $uid_string='') {
        $keepers = array();
        if (!empty($uids)) {
    ...
    function full_search($terms) {
        ...
        if (isset($imap_search_charsets[$this->search_charset])) {
            $charset = $imap_search_charsets[$this->search_charset].' ';
        }
        else {
            $charset = '';
        }
