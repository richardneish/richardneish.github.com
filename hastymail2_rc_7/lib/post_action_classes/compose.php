            ...
            if ($imap->append_start($mailbox, $size)) {
                ...
                if (isset($_SESSION['attachments']) && !empty($_SESSION['attachments'])) {
                    foreach ($_SESSION['attachments'] as $i => $v) {
                        $headers = $message->build_part_header($v['realname'], $v['type'], 'base64');
                        if (substr($path, -1) != $fd) {   
                            $filename = $path.$fd.$i;
                        }   
                        else {      
            ...
            foreach ($post['contacts'] as $string) {
                ...
                foreach ($vcard as $vals) {
                    ...
                    foreach ($vals as $card_id => $atts) {
                        ...
                        foreach ($atts as $props) {
                            if ($props['group'] == 'A' && $props['name'] == 'EMAIL') {
                                $email = '<'.$props['value'].'>';
                            }
                            if ($props['name'] == 'FN') {
                                $name = '"'.$props['value'].'"';
                        ...
                        if (stripslashes($string) == $name.' '.$email || (!$name && stripslashes($string) == $email)) {
                            $found_id = $card_id;
                            break 2;
                        }
                    }
                ...
                if ($found_id) {
                    ...
                    if (is_resource($output_file)) {
                        ...
                        fwrite($output_file, chunk_split(base64_encode($body)));
            ...
            if (!empty($recipients)) {
                ...
                if (!$res) {
                    ...
                    if ($res) {
                        ...
                        $this->form_redirect = true;
                        if (isset($post['reply_box']) && isset($post['reply_uid']) && $post['reply_uid']) {
                            $this->perform_imap_action('ANSWERED', $post['reply_box'], array($post['reply_uid']), false, false);
                        }
                    }
                    if (!isset($post['compose_skip_sent']) || !$post['compose_skip_sent']) {
                        if (isset($_SESSION['user_settings']['sent_folder']) &&
                            isset($_SESSION['folders'][$_SESSION['user_settings']['sent_folder']])) {
                            $email = $message->output_imap_message();
                            $size = $message->get_imap_message_size(strlen($email));
                            if ($imap->append_start($_SESSION['user_settings']['sent_folder'], $size)) {
                                $imap->append_feed($email);
                                if (isset($_SESSION['attachments']) && !empty($_SESSION['attachments'])) {
                                    foreach ($_SESSION['attachments'] as $i => $v) {
                                        $headers = $message->build_part_header($v['realname'], $v['type'], 'base64');
                    ...
                    if (isset($_SESSION['user_settings']['draft_folder']) && isset($post['message_id']) && $post['message_id'] &&
    ...
    if ($user->logged_in) {
        ...
        if (isset($post['attachment_id']) && is_array($post['attachment_id']) && !empty($post['attachment_id'])) {
            foreach ($post['attachment_id'] as $v) {
                $id = stripslashes($v);
                if (isset($_SESSION['attachments'][$id])) {
...
function form_action_compose_attach($form, $post) {
    ...
    if ($user->logged_in) {
        ...
        $res = $this->add_attachemnt('attachment_file', $type);
        if ($res) {
            $this->errors[] = $res;
        }
        else {
