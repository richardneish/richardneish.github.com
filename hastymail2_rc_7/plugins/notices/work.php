...
function notices_init($tools) {
    ...
    $alert = '';
    $tools->add_js('<script type="text/javascript">
     '.$tools->start_cdata().'
    function get_notice_bg() {
    ...
    if ($enable_sound && $sound_file) {
        $tools->add_js('<script type="text/javascript" src="plugins/notices/script/soundmanager2-min.js"></script>');
        $alert .= 'notice_sound();';
    }
    if ($enable_popup) {
        $alert .= 'notice_window();';
    ...
    $js .= $tools->start_cdata().'
    var auto_notice = '.$notice.';
