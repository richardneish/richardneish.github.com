...
$(document).ready(function(){
	...
	$(document).on('click','#intResourceHide', function(evt){
		if(globalActiveApp=='CalDavZAP')
		{
			if((typeof globalInitLoad!='undefined' && !globalInitLoad) && (typeof globalResourceRefresh!='undefined' && !globalResourceRefresh))
				$('#ResizeLoader').show(); 
			...
			$('#CalendarLoader, #ResizeLoader, #MainLoaderInner').animate({left: 25}, transSpeedResource);
			$('#main_h, #searchForm, #main').animate({left: 24}, transSpeedResource, function(){
		...
		else if(globalActiveApp=='CalDavTODO')
		{
			...
			$('#CalendarLoaderTODO, #ResizeLoaderTODO').animate({left: 25, width: 418 + 225}, transSpeedResource);
			$('#main_h_TODO, #searchFormTODO, #mainTODO').animate({left: 24, width: 418 + 225}, transSpeedResource, function(){
				$('#intResourceHide').parent().css('display', 'none');
				$('#intResourceShow').parent().css('display', 'table-cell');
				$('#todoList').resize();
			});
		}
		else if(globalActiveApp=='CardDavMATE')
		{
			$('.resourcesCardDAV_d, #ResourceCardDAVList, #ResourceCardDAVListOverlay').animate({width: 0}, transSpeedResource);
			$('.collection_d, #SearchBox, #ABList, #ABListLoader, #ABListOverlay').animate({left: 25}, transSpeedResource);
			$('.contact_d, #ABMessage, #ABContact, #ABContactOverlay').animate({left: 276}, transSpeedResource);
	...
	$(document).on('click','#intResourceShow', function(evt){
		if(globalActiveApp=='CalDavZAP')
		{
			if((typeof globalInitLoad!='undefined' && !globalInitLoad) && (typeof globalResourceRefresh!='undefined' && !globalResourceRefresh))
				$('#ResizeLoader').show();
			...
			$('#CalendarLoader, #ResizeLoader, #MainLoaderInner').animate({left: 250}, transSpeedResource);
			$('#main_h, #searchForm, #main').animate({left: 249}, transSpeedResource, function(){
				$('#SystemCalDAV .fc-header-title').width($('#main_h_placeholder').width()-$('#SystemCalDAV .fc-header-left').width()-$('#SystemCalDAV .fc-header-right').width()-20);
		...
		else if(globalActiveApp=='CalDavTODO')
		{
			...
			$('#CalendarLoaderTODO, #ResizeLoaderTODO').animate({left: 250, width: 418}, transSpeedResource);
			$('#main_h_TODO, #searchFormTODO, #mainTODO').animate({left: 249, width: 418}, transSpeedResource, function(){
				$('#intResourceShow').parent().css('display', 'none');
				$('#intResourceHide').parent().css('display', 'table-cell');
				$('#todoList').resize();
			});
		...
		else if(globalActiveApp=='CardDavMATE')
		{
			...
			$('.collection_d, #SearchBox, #ABList, #ABListLoader, #ABListOverlay').animate({left: 250}, transSpeedResource);
			$('.contact_d, #ABMessage, #ABContact, #ABContactOverlay').animate({left: 501}, transSpeedResource);
...
$(document).on('drop', '.photo_div', function(event){
	event.stopPropagation();
	event.preventDefault();
...
if(typeof globalEnableKbNavigation=='undefined' || globalEnableKbNavigation!==false)
{
	...
	$(document.documentElement).keydown(function(event)
	{
		...
		{
			// 37 = left, 38 = up, 39 = right, 40 = down
			var selected_contact=null, nextContact=null;
			if((selected_contact=$('#ABList').find('.ablist_item_selected')).length==1)
			{
				if(event.keyCode == 38 && (next_contact=selected_contact.prevAll('.ablist_item').filter(':visible').first()).attr('data-id')!=undefined || event.keyCode == 40 &&  (next_contact=selected_contact.nextAll('.ablist_item').filter(':visible').first()).attr('data-id')!=undefined)
				{
