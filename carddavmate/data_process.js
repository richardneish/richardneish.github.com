...
// FN (extracted from newly created N [previous "process_elem"], required by RFC)
	...
	if(typeof globalContactStoreFN=='string')
		var tmp=globalContactStoreFN.replace(RegExp(',','g'),', ').split(',');
	else	/* new configuration options (arrays) */
		var tmp=globalContactStoreFN.slice();	// copy the configuration array
...
// X-PHONETIC-FIRST-NAME
	if((value=$('[id="vcard_editor"] [data-type="ph_firstname"]').val())!='')
	{
...
// X-PHONETIC-LAST-NAME
	if((value=$('[id="vcard_editor"] [data-type="ph_lastname"]').val())!='')
	{
...
// BDAY
	...
	{
		var valid=true;
		try {var date=$.datepicker.parseDate(globalSessionDatepickerFormat, value)}
		catch (e) {valid=false}
...
// PHOTO
	if(!(value=$('[id="vcard_editor"] [data-type="photo"]').prop('src')).match(RegExp('images/(company|user)\.svg$','')))
	{
		if(vCard.tplM['contentline_PHOTO']!=null && (process_elem=vCard.tplM['contentline_PHOTO'][0])!=undefined)
		{
			...
			if(value.match(RegExp('^data:','i')))
				value=value.replace(RegExp('^data:(?:image/)?','i'),'').replace(RegExp('base64,','i'),'').split(';')[1];	// note: value is now
			process_elem=process_elem.replace('##:::##value##:::##',vcardEscapeValue(value));
		}
		else
		{
			value=value.replace(RegExp('^data:(?:image/)?','i'),'').replace(RegExp('base64,','i'),'').split(';');	// note: value is now array
			process_elem=vCard.tplC['contentline_PHOTO'];
			process_elem=process_elem.replace('##:::##group_wd##:::##','');
			process_elem=process_elem.replace('##:::##params_wsc##:::##',';ENCODING=b;TYPE='+value[0]);
			process_elem=process_elem.replace('##:::##value##:::##',vcardEscapeValue(value[1]));
		}
		vCardText+=process_elem;
	}
...
// X-SOCIALPROFILE
	$('[id="vcard_editor"] [data-type="\\%profile"]').each(
		function (index,element)
		{
			if((value=$(element).find('[data-type="value"]').val())!='')
			{
				...
				process_elem=process_elem.replace('##:::##params_wsc##:::##',params_wsc);
				process_elem=process_elem.replace('##:::##value##:::##', vcardEscapeValue((globalUriHandlerProfile[tmp_type]!=undefined ? globalUriHandlerProfile[tmp_type] : 'x-apple:%u').replace('%u', (tmp_type=='twitter' ? value.replace(/^@+/, '') : value))));
	...
	// folding of long lines (RFC2426 - section 2.6)
	vCardText=vCardText.replace(RegExp('^(.{74})(.*)'),'$1\r\n $2');
...
//			return false;	// vcard UID not present or present more than once
		...
		if(vcard_element!=null)
		{
			...
			$('[data-type="ph_firstname"]').val(vcardUnescapeValue(parsed[4])).change();

			// values not directly supported by the editor (old values are kept intact)
			vCard.tplM['contentline_X-PHONETIC-FIRST-NAME'][0]=vCard.tplC['contentline_X-PHONETIC-FIRST-NAME'];
			vCard.tplM['contentline_X-PHONETIC-FIRST-NAME'][0]=vCard.tplM['contentline_X-PHONETIC-FIRST-NAME'][0].replace(/##:::##group_wd##:::##/g,parsed[1]);
			vCard.tplM['contentline_X-PHONETIC-FIRST-NAME'][0]=vCard.tplM['contentline_X-PHONETIC-FIRST-NAME'][0].replace(/##:::##params_wsc##:::##/g,parsed[3]);
		...
		if(vcard_element!=null)
		{
			...
			$('[data-type="ph_lastname"]').val(vcardUnescapeValue(parsed[4])).change();

			// values not directly supported by the editor (old values are kept intact)
			vCard.tplM['contentline_X-PHONETIC-LAST-NAME'][0]=vCard.tplC['contentline_X-PHONETIC-LAST-NAME'];
			vCard.tplM['contentline_X-PHONETIC-LAST-NAME'][0]=vCard.tplM['contentline_X-PHONETIC-LAST-NAME'][0].replace(/##:::##group_wd##:::##/g,parsed[1]);
			vCard.tplM['contentline_X-PHONETIC-LAST-NAME'][0]=vCard.tplM['contentline_X-PHONETIC-LAST-NAME'][0].replace(/##:::##params_wsc##:::##/g,parsed[3]);
		...
		if(vcard_element!=null)
		{
			...
			if(valid==true)
			{
				$('[data-type="date_bday"]').val(vcardUnescapeValue($.datepicker.formatDate(globalSessionDatepickerFormat, date))).change();
		...
		if(vcard_element!=null)
		{
			...
			if(valid==true)
			{
				$('[data-type="date_anniversary"]').val(vcardUnescapeValue($.datepicker.formatDate(globalSessionDatepickerFormat, date))).change();
		...
		if(vcard_element!=null)	// if the PHOTO attribute is present more than once, we use the first value
		{
			...
			$('[id="vcard_editor"] [data-type="photo"]').css('visibility', 'hidden').prop('src', photo).one('load', function(){
...
//				$('[id="vcard_editor"] [data-type="photo"]').attr('src',parsed[4]);
		...
		else	// use default icons (see X-ABShowAs above)
		{
			if(photo_show_org==true)
				$('#vcard_editor').find('[data-type="photo"]').prop('src','images/company.svg');
			else
				$('#vcard_editor').find('[data-type="photo"]').prop('src','images/user.svg');
		}
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_ADR']))!=null)
		{
			...
			else if(typeof globalAddressCountryEquivalence!='undefined' && globalAddressCountryEquivalence.length>0 && parsed_value[6]!=undefined)	// unknown ADR format (country not detected)
			{
				for(i=0;i<globalAddressCountryEquivalence.length;i++)
					...
					{
						tmp.find('[data-type="country_type"]').children('[data-type="'+jqueryEscapeSelector(globalAddressCountryEquivalence[i].country)+'"]').prop('selected',true);
						break;
					}
			}
			// Note: 
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_EMAIL']))!=null)
		{
			...
			type_values_txt=type_values.unique().sort().join(',');	// TYPE=INTERNET;TYPE=INTERNET;TYPE=HOME; -> array('HOME','INTERNET') -> 'home,internet'
		...
		while((vcard_element=vcard.match(vCard.pre['contentline_X-SOCIALPROFILE']))!=null)
		{
			...
			// click to "add" button if not enought data rows present
			if($('[id="vcard_editor"] [data-type="\\%profile"]').last().find('[data-type="value"]').val()!='')
				$('[id="vcard_editor"] [data-type="\\%profile"]').last().find('[data-type="\\%add"]').find('input[type="image"]').click();
			...
			// get the "TYPE=" values array
			pref=0;	//by default there is no preferred X-SOCIALPROFILE
			type_values=Array();
			j=0;
			social_user='';
			for(i=1;i<parsed_value.length;i++)
			{
				if(parsed_value[i].toLowerCase().indexOf('x-user=')==0)
					social_user=parsed_value[i].replace(/^[^=]+=/,'');
			...
			// if there is no x-user parameter extract the username from the URL
			if(social_user=='')
				social_user=parsed[4].replace(/^.*\/|\/$/g,'')

			type_values_txt=type_values.unique().sort().join(',');	// TYPE=B;TYPE=A;TYPE=C; -> array('B','A','C') -> 'a,b,c'
			type_values_txt_label=type_values.unique().sort().join(' ').replace(RegExp('^:|:$','g'),'');	// TYPE=B;TYPE=A;TYPE=C; -> array('B','A','C') -> 'a b c'

			// if no X-SOCIALPROFILE type defined, we use the 'twitter' type as default
			if(type_values_txt=='')
				type_values_txt=type_values_txt_label='twitter';

			// get the default available types
			type_list=new Array();
			$('[data-type="\\%profile"]:eq('+element_i+')').find('[data-type="profile_type"]').children().each( function(index,element){type_list[type_list.length]=$(element).attr('data-type');});
			// if an existing type regex matches the new type, use the old type
			// and replace the old type definition with new type definition to comforn the server vCard type format
			for(i=0;i<type_list.length;i++)
				if(dataTypes['profile_type'][type_list[i]]!=undefined && type_values_txt.match(RegExp(dataTypes['profile_type'][type_list[i]]))!=null)
				{
					$('[data-type="\\%profile"]').find('[data-type="profile_type"]').find('[data-type="'+type_list[i]+'"]').attr('data-type',type_values_txt);
					break;
				}

			// X-SOCIALPROFILE type: select or append to existing types and select
			select_element=$('[data-type="\\%profile"]:eq('+element_i+') [data-type="profile_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]');
			if(select_element.length==1)
				select_element.prop('selected',true);
			else if(select_element.length==0)
			{
				// create the missing option
				new_opt=$('[id="vcard_editor"] [data-type="profile_type"] :first-child').clone().attr('data-type',type_values_txt).text(type_values_txt_label).wrap('<div>').parent().html();
				// append the option to all element of this type
				$('[id="vcard_editor"] [data-type="profile_type"] :last-child').prev().after(new_opt);
				// select the option on the current type
				$('[data-type="\\%profile"]:eq('+element_i+') [data-type="profile_type"]').find('[data-type="'+jqueryEscapeSelector(type_values_txt)+'"]').prop('selected',true);
			}

			$('[data-type="\\%profile"]:eq('+element_i+') [data-type="value"]').val(vcardUnescapeValue(type_values_txt=='twitter' ? '@'+social_user : social_user)).change();

			// values not directly supported by the editor (old values are kept intact)
			vCard.tplM['contentline_X-SOCIALPROFILE'][element_i]=vCard.tplC['contentline_X-SOCIALPROFILE'];
			vCard.tplM['contentline_X-SOCIALPROFILE'][element_i]=vCard.tplM['contentline_X-SOCIALPROFILE'][element_i].replace(/##:::##group_wd##:::##/g,parsed[1]);
			// if the X-SOCIALPROFILE was preferred, we keep it so (we not support preferred X-SOCIALPROFILE selection directly by editor)
			if(pref==1)
				vCard.tplM['contentline_X-SOCIALPROFILE'][element_i]=vCard.tplM['contentline_X-SOCIALPROFILE'][element_i].replace(/##:::##params_wsc##:::##/g, '##:::##params_wsc##:::##;TYPE=PREF');
