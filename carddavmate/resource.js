...
function CardDAVloadResources(resourceList, forceLoad)
{
	...
	for(var i=0;i<resourceList.length;i++)
		CardDAVnetFindResource(resourceList[i], i.pad((resourceList.length+'').length));
}
	...
	this.loadAddressbookByUID=function(inputUID)
	{
		...
		for(var i=0;i<this.collections.length;i++)
			if(this.collections[i].uid!=undefined && this.collections[i].uid==tmp[1])
			{
				...
				{
					if(typeof globalForceUnloadPrevCollection!='undefined' && globalForceUnloadPrevCollection==true)
					{
						this.collections[i].loaded=false;
...
//									$('#ABList').scrollTop($('#ABList').scrollTop()+selected_contact.offset().top-$('#ABList').offset().top-$('#ABList').height()*globalKBNavigationPaddingRate);
					...
					loadAddressbook(globalResourceCardDAVList.getLoadedAddressbook(),true);
					globalAddressbookIntervalID=setInterval(function() {loadAddressbook(globalResourceCardDAVList.getLoadedAddressbook(),false)}, resourceSettings.syncInterval);
				}
				else
				{
